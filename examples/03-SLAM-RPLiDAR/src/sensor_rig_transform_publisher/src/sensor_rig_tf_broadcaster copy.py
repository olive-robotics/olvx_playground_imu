...
# If you want a convenient Euler->Quaternion function, you can:
# pip install pyquaternion or use geometry_msgs/quaternion from transformations
# or just manually code it:
...
class MultiStaticTFBroadcaster(Node):
    def __init__(self):
        super().__init__('sensor_rig_transform_publisher_node')
    ...
    def load_and_publish_from_yaml(self, path):
        """
        Reads the transforms from the given YAML file and publishes them.
        """
        ...
        if tf_msgs:
            self._broadcaster.sendTransform(tf_msgs)
            self.get_logger().info(f"Published {len(tf_msgs)} static transforms from {path}")
        else:
            self.get_logger().warn(f"No valid transforms to publish from {path}")


def main(args=None):
    rclpy.init(args=args)
    node = MultiStaticTFBroadcaster()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
